...
```bash
chmod 777 $(pwd)
docker run --rm -u ab:$(id -u) -v $(pwd):/a/mm abrainone/ai-linux bash -c "[ -d nn-dataset ] && git -C nn-dataset pull || git -c advice.detachedHead=false clone --depth 1 https://github.com/ABrain-One/nn-dataset"
```

Running script
```bash
docker run --rm -u ab:$(id -u) --shm-size=16G -v $(pwd)/nn-dataset:/a/mm abrainone/ai-linux bash -c ". run50.sh"
```

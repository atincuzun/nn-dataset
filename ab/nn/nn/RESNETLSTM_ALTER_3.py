...
class SpatialAttentionLSTMDecoder(nn.Module):
    ...
    def forward(self, features, captions, hidden_state=None):
        ...
        for t in range(seq_len):
            ...
            logits = self.fc(self.dropout(h))                     # [B, vocab_size]
    ...
    def learn(self, train_data):
        ...
        for i, (images, captions) in enumerate(train_data):
            ...
            with autocast(device_type=self.device.type):
                outputs, targets = self.forward(images_exp, captions_exp, hidden_state)
                loss = self.criteria[0](outputs.contiguous().view(-1, outputs.shape[2]), targets.contiguous().view(-1))
